...
#To make the shorter object name
 All_T= All_T_WSG
...
defauanSimWSG.bins = function(DM1,Bin1,Bin2){ 
  ...
  for(i in 1:SimT){
    ...
    if(length(NL)>=length(L)){
      ...
      #Assumption: not constant tree size
      #Replace with new trees from the other group
    AGB_Sim = sum(mapply(AGB,All_NL$dbh.C3[Sim_Index],All_NL$WSG[Sim_Index]))+
...
save.image("Sim_All_Primates_Cont_NotConstant.RData")
